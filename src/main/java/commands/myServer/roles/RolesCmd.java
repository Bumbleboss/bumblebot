	...
	public void doCommand(CommandEvent e) {
		...
		for(int i = 0; i < list.size(); i++) {
			if(RolesManager.getRoleType(i).equals("perks")) {
				eb.append("***"+RolesManager.getRoleName(i)).append("*** - *").append(RolesManager.getRoleDes(i)+"*\n");
			}
			if(RolesManager.getRoleType(i).equals("gender")) {
				eb1.append("***"+RolesManager.getRoleName(i)).append("*** - *").append(RolesManager.getRoleDes(i)+"*\n");
			}
			if(RolesManager.getRoleType(i).equals("special")) {
				eb2.append("***"+RolesManager.getRoleName(i)).append("*** - *").append(RolesManager.getRoleDes(i)+"*\n");
			}
		}
		...
		e.reply(new EmbedBuilder().addField("Usage", "To add a role, type **" + ConfigUtil.getPrefix()+ new AddRolesCmd().getName() + " <role_name>**", false)
				...
				.addField("Custom", "Only given to whom is trusted/active on the server.", false)
